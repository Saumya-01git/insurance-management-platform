...
const EditPolicyPage = ({ policy, onCancel, onSubmitSuccess }) => {
  ...
  useEffect(() => {
    if (policy) {
      setFormData({
        ...
        policyType: policy.policyType || "Commercial Property",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      if (onSubmitSuccess) {
        await onSubmitSuccess(policy.id, formData);
      } else {
      ...
      <div className="flex items-center gap-3 pb-3 border-b border-slate-200/80 dark:border-slate-800">
        ...
        <div>
          <nav className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
            ...
            <span>/</span>
            <span className="font-bold text-slate-900 dark:text-white">Edit Policy</span>
          </nav>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight mt-0.5">
            Edit Policy: {formatPolicyNumber(policy?.policyNumber || policy?.id)}
          </h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-[#0C1424] border border-slate-200/80 dark:border-white/10 shadow-xl space-y-8">
        <div className="space-y-4">
          <div className="flex items-center gap-2 pb-2 border-b border-slate-100 dark:border-slate-800">
            ...
            <h3 className="text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider">
              1. Policy & Underwriting Agreement
            </h3>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            ...
            <span>{submitting ? "Updating..." : "Update Policy Agreement"}</span>
          </button>
        </div>
      </form>
    </div>
