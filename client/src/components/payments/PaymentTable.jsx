      ...
      <table className="w-full text-left border-collapse min-w-[1050px]">
        <thead>
          <tr className="border-b border-slate-200/80 dark:border-slate-800/80 bg-slate-50/80 dark:bg-slate-900/50 text-[11px] font-extrabold uppercase tracking-wider text-slate-400">
            <th className="py-4 px-5 min-w-[120px]">Payment ID</th>
            <th className="py-4 px-5 min-w-[170px]">Customer</th>
            <th className="py-4 px-5 min-w-[130px]">Policy #</th>
            <th className="py-4 px-5 min-w-[130px]">Amount</th>
            <th className="py-4 px-5 min-w-[160px]">Payment Method</th>
            <th className="py-4 px-5 min-w-[160px]">Transaction ID</th>
            <th className="py-4 px-5 min-w-[120px]">Settlement Date</th>
          ...
          {payments.map((pay) => {
            ...
            return (
              <tr key={pay.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                ...
                {/* Amount */}
                <td className="py-4.5 px-5 font-black text-[#10B981] dark:text-emerald-400 whitespace-nowrap">
                  {formatCurrency(pay.amount)}
                </td>

                {/* Payment Method */}
                <td className="py-4.5 px-5 text-slate-700 dark:text-slate-200 font-medium whitespace-nowrap">
                  {pay.paymentMethod}
                </td>

                {/* Transaction ID */}
                <td className="py-4.5 px-5 font-mono text-slate-500 dark:text-slate-400 text-[11px] whitespace-nowrap">
