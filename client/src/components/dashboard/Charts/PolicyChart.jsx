...
const policyData = [
  { type: "Health", count: 9840, color: "#2563EB" },
  { type: "Auto", count: 7210, color: "#06B6D4" },
  { type: "Life", count: 4850, color: "#3B82F6" },
  { type: "Property", count: 2683, color: "#8B5CF6" },
];
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-slate-900/95 backdrop-blur-md border border-slate-700 p-3 rounded-xl shadow-xl text-white text-xs space-y-1">
        ...
        <p className="text-cyan-400 font-extrabold">{data.count.toLocaleString()} Active Policies</p>
      </div>
    );
  }
  return null;
    ...
    <div className="p-5 sm:p-6 rounded-2xl bg-white dark:bg-[#0c1424] border border-slate-200/80 dark:border-white/10 shadow-sm space-y-5">
      ...
      <div className="h-64 w-full pt-2">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={policyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
