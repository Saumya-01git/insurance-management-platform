...
const StatCard = ({ title, value, change, isPositive, icon: Icon, color = "blue", description }) => {
  ...
  return (
    <div className="p-6 pb-6 rounded-2xl bg-white dark:bg-[#0c1424] border border-slate-200/80 dark:border-white/10 shadow-sm hover:shadow-md transition-all duration-200 space-y-3.5">
      <div className="flex items-center justify-between">
        <span className="text-xs font-black uppercase tracking-wider text-slate-500 dark:text-slate-400">
          {title}
        </span>
        <div className={`w-11 h-11 rounded-2xl ${activeColor.bg} ${activeColor.text} ${activeColor.border} border flex items-center justify-center shrink-0 shadow-sm`}>
      ...
      {description && (
        <p className="text-xs text-slate-400 pt-2 border-t border-slate-100 dark:border-slate-800/80 font-medium leading-normal">
          {description}
        </p>
      )}
    </div>
