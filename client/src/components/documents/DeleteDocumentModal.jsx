...
const DeleteDocumentModal = ({ isOpen, onClose, onConfirm, document }) => {
      ...
      <div className="w-full max-w-md rounded-3xl bg-white dark:bg-[#0C1424] border border-slate-200/80 dark:border-white/10 shadow-2xl overflow-hidden p-6 space-y-5">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
            Are you sure you want to delete <strong className="text-slate-900 dark:text-white">{document.title}</strong> (<span className="font-mono text-[#2563EB] dark:text-cyan-400">{document.documentId || document.id}</span>)?
          </p>
          <div className="p-3 rounded-xl bg-rose-50/60 dark:bg-rose-950/30 border border-rose-200/60 dark:border-rose-900/30 text-[11px] text-rose-700 dark:text-rose-300 font-semibold space-y-1">
            <p>⚠️ Warning: This document will be permanently unlinked from carrier vault storage.</p>
          </div>
        </div>
