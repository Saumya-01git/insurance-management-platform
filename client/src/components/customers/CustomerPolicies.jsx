  ...
  if (!policies || policies.length === 0) {
    return (
      <div className="p-8 text-center bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-200/80 dark:border-slate-700/50 space-y-2">
        ...
        <h5 className="text-xs font-bold text-slate-700 dark:text-slate-300">No Policies Underwritten</h5>
        <p className="text-[11px] text-slate-400">This customer currently has no active policies assigned.</p>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-extrabold text-slate-900 dark:text-white tracking-tight">
            Underwritten Policies ({policies.length})
          </h3>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-100 dark:border-slate-800 text-[11px] font-extrabold uppercase tracking-wider text-slate-400">
              <th className="pb-3 pr-4">Policy ID</th>
              <th className="pb-3 px-4">Coverage Type</th>
              <th className="pb-3 px-4">Coverage Amount</th>
              <th className="pb-3 px-4">Annual Premium</th>
              <th className="pb-3 px-4">Status</th>
              <th className="pb-3 pl-4 text-right">Renewal Date</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs font-semibold">
            {policies.map((pol) => (
              <tr key={pol.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                ...
                <td className="py-3.5 px-4">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-50 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900/30">
                    {pol.status}
                  </span>
                </td>
                <td className="py-3.5 pl-4 text-right text-slate-500 dark:text-slate-400 font-medium">
                  {formatDate(pol.renewalDate)}
                </td>
              </tr>
            ))}
          </tbody>
