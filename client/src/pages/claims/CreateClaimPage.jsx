...
const CreateClaimPage = ({ onCancel, onSubmitSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.customer || !formData.policyNumber || !formData.claimAmount) {
      toast.error("Please fill in required fields: Customer, Policy #, and Claim Amount.");
      return;
    }
      ...
      <div className="flex items-center gap-3 pb-3 border-b border-slate-200/80 dark:border-slate-800">
        ...
        <div>
          <nav className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 font-medium">
            ...
            <span>/</span>
            <span className="font-bold text-slate-900 dark:text-white">Register Claim</span>
          </nav>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight mt-0.5">
            Register New Claim Loss Filing
          </h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-[#0C1424] border border-slate-200/80 dark:border-white/10 shadow-xl space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 mb-1">
              Claimant Customer Name <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
              name="customer"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 mb-1">
              Associated Policy Number <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
              name="policyNumber"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 mb-1">
              Loss Category / Claim Type <span className="text-rose-500">*</span>
            </label>
            <select
              name="claimType"
            ...
            >
              ...
              <option value="Disability Indemnity">Disability Indemnity</option>
        ...
        <div className="space-y-1">
          ...
          <textarea
            ...
            onChange={handleChange}
            placeholder="Describe the loss incident narrative..."
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            ...
            <span>{submitting ? "Submitting..." : "Submit Claim Filing"}</span>
          </button>
        </div>
      </form>
    </div>
